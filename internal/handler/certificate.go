...
func (h *CertificateHandler) Verify(c *gin.Context) {
	...
	if err != nil {
		// If cert was returned (with error info), return it along with the error
		if cert != nil {
...
// Retry handles POST /api/v1/certificates/:id/retry
// Re-creates the ACME order for a failed certificate with new challenges.
func (h *CertificateHandler) Retry(c *gin.Context) {
	...
	cert, err := h.svc.RetryVerification(id)
	if err != nil {
		response.InternalError(c, err)
		return
	}
...
func (h *CertificateHandler) Revoke(c *gin.Context) {
	...
	if err := h.svc.RevokeCertificate(id); err != nil {
		response.InternalError(c, err)
		return
	}

	response.OK(c, "certificate revoked successfully")
}
