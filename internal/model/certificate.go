...
// ChallengeType represents the ACME challenge type used for domain validation.
...
// CAEnvironment represents the CA environment (production or staging).
type CAEnvironment string

const (
	CAEnvProduction CAEnvironment = "production"
	CAEnvStaging    CAEnvironment = "staging"
)
...
type Certificate struct {
	...
	ChallengeMode ChallengeType     `gorm:"type:varchar(20);not null;default:dns-01" json:"challenge_type"`
	CAEnv         CAEnvironment     `gorm:"type:varchar(20);not null;default:production" json:"ca_env"`
	Status        CertificateStatus `gorm:"type:varchar(20);not null;default:pending" json:"status"`
	ErrorMessage  string            `gorm:"type:text" json:"error_message,omitempty"`               // Last error message for failed status
	OrderURL      string            `gorm:"type:varchar(512)" json:"order_url,omitempty"`           // ACME order URL
	OrderExpiresAt *time.Time       `json:"order_expires_at,omitempty"`                             // ACME order expiration time
