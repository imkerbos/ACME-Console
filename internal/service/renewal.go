...
func (s *RenewalService) initiateRenewal(cert *model.Certificate) error {
	...
	if s.certSvc.useLego && s.certSvc.legoSvc != nil {
		if err := s.certSvc.legoSvc.CreateOrderWithOptions(cert.ID, cert.Email, domains, string(cert.KeyType), cert.KeySize, string(cert.ChallengeMode), string(cert.CAEnv)); err != nil {
...
func (s *RenewalService) processPendingCertificates() error {
	...
	for _, cert := range certs {
		// Check if order has expired (use actual order_expires_at if available, fallback to 7 days from last_renewal_at)
		orderExpired := false
		if cert.OrderExpiresAt != nil {
			orderExpired = time.Now().After(*cert.OrderExpiresAt)
		} else if cert.LastRenewalAt != nil {
			orderExpired = time.Since(*cert.LastRenewalAt) > 7*24*time.Hour
		}

		if orderExpired {
			logger.Info("Renewal order expired, resetting",
				logger.Uint("cert_id", cert.ID),
			)
			s.updateRenewalStatus(cert.ID, model.RenewalStatusIdle, false)
			s.logRenewal(cert.ID, "reset", "success", "Order expired, will retry with new order", cert.ExpiresAt, nil)
			continue
		}
