...
func NewNotificationService(db *gorm.DB, settingSvc *SettingService) *NotificationService {
	return &NotificationService{db: db, settingSvc: settingSvc}
}
...
func (s *NotificationService) sendGenericWebhook(config *model.NotificationConfig, cert *model.Certificate, daysLeft int) error {
	...
	if certURL := s.settingSvc.GetCertificateURL(cert.ID); certURL != "" {
		payload["url"] = certURL
	}
...
func (s *NotificationService) sendTelegramNotification(config *model.NotificationConfig, cert *model.Certificate, daysLeft int) error {
	...
	// Build link line if site URL is configured
	linkLine := ""
	if certURL := s.settingSvc.GetCertificateURL(cert.ID); certURL != "" {
		linkLine = fmt.Sprintf("\n🔗 <b>View & Renew:</b> <a href=\"%s\">Open Certificate</a>\n", certURL)
	}
...
func (s *NotificationService) sendLarkNotification(config *model.NotificationConfig, cert *model.Certificate, daysLeft int) error {
	...
	// Add action button if site URL is configured
	if certURL := s.settingSvc.GetCertificateURL(cert.ID); certURL != "" {
