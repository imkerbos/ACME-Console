...
func (s *SettingService) GetSiteConfig() SiteSettings {
	return SiteSettings{
		...
		URL:      s.Get(model.SettingSiteURL),
	}
}

// GetCertificateURL 获取证书详情页链接
func (s *SettingService) GetCertificateURL(certID uint) string {
	siteURL := s.Get(model.SettingSiteURL)
	if siteURL == "" {
		return ""
	}
	// 去除尾部斜杠
	...
	return fmt.Sprintf("%s/certificates/%d", siteURL, certID)
}
