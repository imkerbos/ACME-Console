      ...
      <form @submit.prevent="handleSubmit" class="create-form">
        ...
        <div class="form-section">
          ...
          <p class="form-hint">
            ...
            {{ $t('certificate.emailHint') }}
          </p>
        </div>

        <div class="form-section">
          <label class="form-label">
            <span class="label-text">{{ $t('certificate.challengeType') }}</span>
          </label>
          <div class="key-type-options">
            <label class="key-type-option" :class="{ selected: challengeType === 'dns-01' }">
              <input type="radio" v-model="challengeType" value="dns-01" />
              <div class="option-content">
                <div class="option-header">
                  <span class="option-title">DNS-01</span>
                  <span class="option-badge recommended">{{ $t('certificate.issueModeDefault') }}</span>
                </div>
                <p class="option-description">{{ $t('certificate.dns01Desc') }}</p>
              </div>
            </label>
            <label class="key-type-option" :class="{ selected: challengeType === 'http-01', disabled: hasWildcard }">
              <input type="radio" v-model="challengeType" value="http-01" :disabled="hasWildcard" />
              <div class="option-content">
                <div class="option-header">
                  <span class="option-title">HTTP-01</span>
                </div>
                <p class="option-description">{{ $t('certificate.http01Desc') }}</p>
                <p v-if="hasWildcard" class="option-warning">{{ $t('certificate.http01WildcardWarning') }}</p>
              </div>
            </label>
          </div>
        </div>
        ...
        <div class="advanced-toggle" @click="showAdvanced = !showAdvanced">
          <svg :class="{ rotated: showAdvanced }" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M9 5l7 7-7 7"/>
          </svg>
          <span>{{ $t('certificate.advancedOptions') }}</span>
          <span v-if="caEnv !== 'production' || keyType !== 'RSA'" class="advanced-changed-hint">{{ $t('certificate.advancedChanged') }}</span>
        </div>
        ...
        <div v-show="showAdvanced" class="advanced-section">
              ...
              <label class="key-type-option" :class="{ selected: keyType === 'RSA' }">
                ...
                <div class="option-content">
                  <div class="option-header">
                    ...
                    <span class="option-badge recommended">2048-bit</span>
                  </div>
              ...
              <label class="key-type-option" :class="{ selected: keyType === 'ECC' }">
                ...
                <div class="option-content">
                  <div class="option-header">
                    ...
                    <span class="option-badge">P-256</span>
                  </div>
          ...
          <div class="form-section">
            <label class="form-label">
              <span class="label-text">{{ $t('certificate.caEnv') }}</span>
            </label>
            <div class="key-type-options">
              <label class="key-type-option" :class="{ selected: caEnv === 'production' }">
                <input type="radio" v-model="caEnv" value="production" />
                <div class="option-content">
                  <div class="option-header">
                    <span class="option-title">Production</span>
                    <span class="option-badge recommended">{{ $t('certificate.issueModeDefault') }}</span>
                  </div>
                  <p class="option-description">{{ $t('certificate.caEnvProductionDesc') }}</p>
                </div>
              </label>
              <label class="key-type-option" :class="{ selected: caEnv === 'staging' }">
                <input type="radio" v-model="caEnv" value="staging" />
                <div class="option-content">
                  <div class="option-header">
                    <span class="option-title">Staging</span>
                    <span class="option-badge">{{ $t('certificate.caEnvStagingBadge') }}</span>
                  </div>
                  <p class="option-description">{{ $t('certificate.caEnvStagingDesc') }}</p>
                </div>
              </label>
            </div>
          </div>
...
const challengeType = ref('dns-01')
const caEnv = ref('production')
...
// Auto-switch to DNS-01 if wildcard is detected
watch(hasWildcard, (val) => {
  if (val && challengeType.value === 'http-01') {
