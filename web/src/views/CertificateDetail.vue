<template>
  <div class="certificate-detail">
    ...
    <template v-else-if="certificate">
        ...
        <div class="header-content">
          <div class="header-main">
            <div class="cert-icon" :class="`icon-${certificate.status}`">
              ...
              <svg v-else-if="certificate.status === 'revoked'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          <div class="header-actions">
            <!-- DNS check button (only for dns-01) -->
            <button
              v-if="(certificate.status === 'pending' || certificate.status === 'failed') && certificate.challenge_type !== 'http-01'"
            ...
            >
              ...
              {{ checking ? $t('certificate.checking') : $t('certificate.checkDNS') }}
            </button>
            ...
            <button
              ...
              @click="handleRetry"
            >
              <span v-if="retrying" class="btn-spinner"></span>
              <svg v-else viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
              </svg>
              {{ retrying ? $t('common.loading') : $t('certificate.retry') }}
            </button>
            <!-- Verify button -->
            <button
      ...
      <div v-if="certificate.ca_env === 'staging'" class="alert alert-info" style="margin-bottom: 1.5rem;">
        <svg class="alert-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <circle cx="12" cy="12" r="10"/>
          <path d="M12 16v-4M12 8h.01"/>
        </svg>
        <span>{{ $t('certificate.stagingWarning') }}</span>
      </div>

      <!-- Error Message -->
      <div v-if="certificate.error_message" class="alert alert-error" style="margin-bottom: 1.5rem;">
        <svg class="alert-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <circle cx="12" cy="12" r="10"/>
          <path d="M12 8v4M12 16h.01"/>
        </svg>
        <div class="alert-content">
          <span>{{ certificate.error_message }}</span>
        </div>
      </div>

      <!-- Order Expiry Warning -->
      <div v-if="orderExpiryWarning" class="alert alert-warning-box" style="margin-bottom: 1.5rem;">
        <svg class="alert-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
        </svg>
        <span>{{ orderExpiryWarning }}</span>
      </div>
      ...
      <div v-if="certificate.challenges?.length > 0" class="detail-card">
        <div class="card-header-flex">
          <div class="card-title">
            ...
            {{ isHTTP01 ? $t('challenge.httpChallenge') : $t('challenge.dnsChallenge') }}
          </div>
        ...
        <div v-if="certificate.status === 'pending'" class="challenge-hint">
          ...
          {{ isHTTP01 ? $t('challenge.httpChallengeHint') : $t('challenge.challengeHint') }}
        </div>
        ...
        <div class="challenge-list">
          <div v-for="challenge in sortedChallenges" :key="challenge.id" class="challenge-item">
            ...
            <!-- DNS-01 records -->
            <div v-if="challenge.type !== 'http-01'" class="challenge-records">
            ...
            <!-- HTTP-01 records -->
            <div v-else class="challenge-records">
              <div class="record-row">
                <span class="record-label">{{ $t('challenge.httpPath') }}</span>
                <code class="record-value">{{ challenge.http_path }}</code>
                <button class="copy-btn" @click="copyToClipboard(challenge.http_path)" :title="$t('common.copy')">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <rect x="9" y="9" width="13" height="13" rx="2" ry="2"/>
                    <path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/>
                  </svg>
              ...
              <div class="record-row">
                <span class="record-label">{{ $t('challenge.httpContent') }}</span>
                <code class="record-value">{{ challenge.http_content }}</code>
                <button class="copy-btn" @click="copyToClipboard(challenge.http_content)" :title="$t('common.copy')">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <rect x="9" y="9" width="13" height="13" rx="2" ry="2"/>
                    <path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/>
                  </svg>
      ...
      <!-- Revoke Card (when ready) -->
      <div v-if="certificate.status === 'ready'" class="detail-card">
        <div class="card-title">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"/>
          </svg>
          {{ $t('certificate.revokeCert') }}
        </div>
        <p class="revoke-desc">{{ $t('certificate.revokeDesc') }}</p>
        <div class="revoke-actions">
          <button v-if="!revokeConfirmed" class="btn btn-danger-outline btn-sm" @click="revokeConfirmed = true">
            {{ $t('certificate.revokeBtn') }}
          </button>
          <template v-else>
            <span class="revoke-confirm-text">{{ $t('certificate.revokeConfirm') }}</span>
            <button class="btn btn-danger btn-sm" :disabled="revoking" @click="handleRevoke">
              <span v-if="revoking" class="btn-spinner"></span>
              {{ revoking ? $t('common.loading') : $t('common.confirm') }}
            </button>
            <button class="btn btn-secondary btn-sm" @click="revokeConfirmed = false">{{ $t('common.cancel') }}</button>
          </template>
        </div>
      </div>
...
const isHTTP01 = computed(() => {
  return certificate.value?.challenge_type === 'http-01'
})

const orderExpiryWarning = computed(() => {
  if (!certificate.value) return null
  if (certificate.value.status !== 'pending' && certificate.value.status !== 'failed') return null
  if (!certificate.value.order_expires_at) return null

  const expires = new Date(certificate.value.order_expires_at)
  const now = new Date()
  const hoursLeft = (expires - now) / (1000 * 60 * 60)

  if (hoursLeft <= 0) {
    return t('certificate.orderExpired')
  } else if (hoursLeft <= 24) {
    return t('certificate.orderExpiringSoon', { hours: Math.ceil(hoursLeft) })
  } else {
    const daysLeft = Math.ceil(hoursLeft / 24)
    return t('certificate.orderExpiresIn', { days: daysLeft })
  }
})
...
async function handleRetry() {
  ...
  try {
    const response = await certificateApi.retry(id)
    certificate.value = response.data
    dnsCheckResults.value = null
  } catch (e) {
    error.value = e.message
...
function copyAllChallenges() {
  ...
  let text
  if (isHTTP01.value) {
    text = sortedChallenges.value.map(c =>
      `# ${c.domain}\n# Path: ${c.http_path}\n${c.http_content}`
    ).join('\n\n')
  } else {
    text = sortedChallenges.value.map(c =>
