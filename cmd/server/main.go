...
func main() {
	...
	// Initialize database (retry up to 30 times, waiting 2s between attempts)
	var db *gorm.DB
	for i := 1; i <= 30; i++ {
	...
	if err != nil {
		logger.Fatal("Failed to initialize database after retries", logger.Err(err))
	}
	...
	notificationSvc := service.NewNotificationService(db, settingSvc)
